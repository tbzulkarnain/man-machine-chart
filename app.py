...
st.caption("Aplikasi Fleksibel untuk Berbagai Karakteristik & Urutan Proses Mesin")
...
st.info(
    "💡 **Petunjuk Fleksibilitas:**\n"
    "- Pilih **Type** yang sesuai agar sistem dapat menyusun linimasa dengan presisi.\n"
    "- Gunakan **Load / Swap** jika bongkar-pasang palet dilakukan sekaligus dalam 1 kunjungan ke mesin."
)
...
if valid_rows:
    # 1. HITUNG ELEMEN LINGKUP IE
    man_work_per_mc = sum(r["Duration"] for r in valid_rows if r["Actor"] in ["Man", "Both"])
    
    # Cycle time mesin = Waktu elemen 'Both' + Waktu elemen 'Machine'
    ...
    if man_work_per_mc > 0 and mc_cycle_time > 0:
        ...
        # 3. GENERATOR PROCESS SHEET OTOMATIS BERDASARKAN SEQUENCE
        chart_rows = []
        acc = 0.0
        
        # Generasi linimasa umum berurutan berdasarkan sequence yang di-input
        for loop in range(2): # 2 Siklus untuk visualisasi
            for m_idx in range(1, num_machines + 1):
                for elem in valid_rows:
                    acc = round(acc + elem["Duration"], 2)
                    r_dict = {
                        "Time (s)": acc, 
                        "Operator": f"[MC {m_idx}] {elem['Process']}", 
